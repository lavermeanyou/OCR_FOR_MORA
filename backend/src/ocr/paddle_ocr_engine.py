...
class PaddleOCREngine:
    ...
    def _preprocess_image(self, image_path: str) -> str:
        """
        이미지가 너무 크면 리사이즈.
        PaddleOCR은 고해상도 이미지에서 텍스트 검출 실패하는 문제가 있음.
        """
        img = Image.open(image_path)
        max_dim = max(img.size) # size의 return값은 (가로,세로)
        ...
        img_resized = img.resize(new_size, Image.LANCZOS) # LANCZOS 리사이즈 메소드
    ...
    def extract(self, image_path: str) -> dict:
        ...
        for page_idx, res in enumerate(results): # enumerate 인덱스값 주는거
            # Result 객체는 json 속성 제공
